...
const cartsController = require("../compoments/carts/controller");
const phonesController = require("../compoments/phones/controller");
...
router.get("/list-cart", async function (req, res, next) {
  // lấy danh sách sản phẩm từ database và hiển thị
  const data = await cartsController.getCarts();
  

  res.render("carts", { carts: data });
  //hiển thị(render)
});
router.get("/list-phone", async function (req, res, next) {
  // lấy danh sách sản phẩm từ database và hiển thị
  const data = await phonesController.getphone();
  

  res.render("phones", { phones: data });
  //hiển thị(render)
});
...
router.get("/phone/them-moi",  async function (req, res, next) {
  // lấy danh sách của danh mục

  res.render("phones_insert");
...
router.get("/:id/edit", async function (req, res, next) {
  ...
  const categories = await categoryController.getselectategories(product.category_id);
